...
router.post('/', upload.single('image'), async (req, res) => {
  try {
    ...
    req.body.imageUrl = req.file ? `/uploads/${req.file.filename}` : null;
...
router.get('/', async (req, res) => {
  try {
    const lists = await List.find({}).populate('author').sort({ createdAt: 'desc' });
...
router.put('/:listId', upload.single('image'), async (req, res) => {
  try {
    ...
    const updatedList = await List.findByIdAndUpdate(req.params.listId, req.body, { new: true });
...
router.get('/:listId', async (req, res) => {
  try {
    const list = await List.findById(req.params.listId).populate('author').populate('reviews.author');
...
router.get('/:listId/reviews', async (req, res) => {
  try {
    const list = await List.findById(req.params.listId).populate('reviews.author');
